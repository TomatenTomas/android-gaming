...
public class GLPixelFont {
    ...
    private Mesh[] _glyphs = new Mesh[Config.FONT_CHAR_COUNT]; //a vertex buffer for each glyph, for rendering with OpenGL.

    public GLPixelFont() {
        for (int c = 0; c < Config.FONT_CHAR_COUNT; c++) {
            _glyphs[c] = null;
        }
    }
    ...
    public Mesh getChar(char c){
        ...
        if(c < Config.FONT_OFFSET || c >= Config.FONT_OFFSET+Config.FONT_CHAR_COUNT){
            return null;
        }
        int i = c - Config.FONT_OFFSET;
        if(_glyphs[i] == null){
    ...
    private Mesh createMeshForGlyph(final char c){
        Utils.require(c >= Config.FONT_OFFSET && c < Config.FONT_OFFSET+Config.FONT_CHAR_COUNT);
        float[] vertices = new float[Config.FONT_HEIGHT*Config.FONT_WIDTH*Mesh.COORDS_PER_VERTEX];
        final float z = 0;
        final int charIndex = c-Config.FONT_OFFSET;
        int i = 0;
        for (int y = 0; y < Config.FONT_HEIGHT; y++) {
            for (int x = 0; x < Config.FONT_WIDTH; x++) {
                final int index = (Config.FONT_HEIGHT*Config.FONT_WIDTH) * charIndex + Config.FONT_WIDTH * y + x;
